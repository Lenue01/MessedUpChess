...
def draw_board():
    ...
    if selected_pos:
        pygame.draw.rect(
            screen,
            YELLOW,
            (selected_pos[1] * SQUARE_SIZE, selected_pos[0] * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE),
            3 
        )
...
REMOVE_INTERVAL = 60  # 1 minutes (in seconds)

def remove_random_piece():
    # choose from all pieces except kings
    pieces = [(row, col) for row in range(8) for col in range(8) 
              if board[row][col]]
    if not pieces:
        return  # nothing left to remove
    row, col = random.choice(pieces)
    print(f"Removing {board[row][col].color} {board[row][col].name} at {row}, {col})")
    board[row][col] = None
...
def main():
    ...
    while True:
        ...
        # check if it's time to remove a piece
        current_time = time.time()
        if current_time - last_remove_time >= REMOVE_INTERVAL:
            remove_random_piece()
